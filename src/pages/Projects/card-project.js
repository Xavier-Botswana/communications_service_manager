...
const CardProject = (props) => {
  ...
  const PATCH_URL = `https://sheet.best/api/sheets/60a3969d-8d9e-4b41-80b0-3f359e8dbb6e/tabs/individualsdeliveries/${
    indeliveries.id - 1
  }`;
  ...
  const handleDispatch = () => {
    ...
    firebase.logAction(
      currentUser.email,
      `Dispatched individual delivery request from ${indeliveries.username}`
    );
  };
  ...
  const handleDecline = () => {
    ...
    firebase.logAction(
      currentUser.email,
      `Declined individual delivery request from ${indeliveries.username}`
    );
  };
